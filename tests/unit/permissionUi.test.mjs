...
test("权限卡：permissionResolved → 只摘对应那张卡（他实例作答/超时/该轮结束）", () => {
  let s = reduceHostMessage(st0(), cardMsg("r1", "Bash", "ls"));
  s = reduceHostMessage(s, cardMsg("r2", "Write", "/w/a.txt"));
  ...
  // 未知/畸形 id → 原样返回（本实例没这张卡，别白重建 state 触发渲染）
  assert.equal(
    reduceHostMessage(s, { type: "permissionResolved", requestId: "nope" }),
    s,
  );
  assert.equal(
    reduceHostMessage(s, { type: "permissionResolved", requestId: "" }),
    s,
  );
  assert.equal(reduceHostMessage(s, { type: "permissionResolved" }), s);
});
