...
// 复查修-7：并入按最长公共段对齐——已存头部被 50 条上限挤掉、新来的是残份时，
// 逐条前缀对齐会错位成整段拼接（重复 + 把旧条目排到最新之后，↑ 翻出的“最新一条”是旧的）
test("复查修-7 并入：前缀被上限挤掉 / incoming 是残份 → 只补尾部，不重复、不挤掉最新", () => {
  // 已存被上限挤掉最老的 a；另一实例的残份从 a 开头、又缺 d/e 这两个最新的
  assert.deepEqual(
    mergeHistoryEntries(["b", "c", "d", "e"], ["a", "b", "c", "f"]),
    ["b", "c", "d", "e", "f"],
  );
  // 幂等：同一 incoming 并两次结果不变
  const once = mergeHistoryEntries(["b", "c", "d", "e"], ["a", "b", "c", "f"]);
  assert.deepEqual(mergeHistoryEntries(once, ["a", "b", "c", "f"]), once);
  // incoming 是残份（只记得最老一条）→ 已存条目一条不丢
  assert.deepEqual(mergeHistoryEntries(["一", "二", "三"], ["一"]), [
    "一",
    "二",
    "三",
  ]);
  // incoming 是「已存 + 新发的几条」（正常路径）→ 尾部照常接上
  assert.deepEqual(mergeHistoryEntries(["一", "二"], ["一", "二", "三"]), [
    "一",
    "二",
    "三",
  ]);
  // 两份互不相识 → 拼接兜底（历史只增不减）
  assert.deepEqual(mergeHistoryEntries(["x"], ["y"]), ["x", "y"]);
});

test("复查修-7 并入：store.save 端到端——残份不抹已存、上限裁剪不错位", async () => {
  const { store, fs } = makeStore();
  await store.save("S1", ["一", "二", "三"]);
  await store.flush();
  await store.save("S1", ["二", "三", "四"]); // 另一实例的副本（少了最老的「一」）
  await store.flush();
  const parsed = JSON.parse(fs.files.get(FILE) as string) as {
    sessions: Record<string, string[]>;
  };
  assert.deepEqual(parsed.sessions.S1, ["一", "二", "三", "四"]);
});
