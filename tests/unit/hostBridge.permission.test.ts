...
test("permission: permissionSettled → permissionResolved 广播给全部实例（他实例摘卡）；空 id 忽略", async () => {
  const deps = makeDeps();
  const bridge = createHostBridge(deps);
  const winA = makeWin();
  const winB = makeWin();
  handshake(deps, bridge, winA);
  handshake(deps, bridge, winB);
  await tick();
  // 两个实例都在：卡广播是全局的，摘卡通知也必须全局（否则后台实例留残影——用户报障原形）
  bridge.permissionSettled("req-9");
  const msgs = deps.sent.filter((s) => s.msg.type === "permissionResolved");
  assert.equal(msgs.length, 2);
  assert.deepEqual(msgs[0].msg, {
    type: "permissionResolved",
    requestId: "req-9",
  });
  assert.notEqual(msgs[0].win, msgs[1].win);
  // 空 id（非法调用）→ 忽略 + log：摘卡按 requestId 匹配，空串通知毫无意义
  bridge.permissionSettled("");
  assert.equal(
    deps.sent.filter((s) => s.msg.type === "permissionResolved").length,
    2,
  );
  assert.ok(
    deps.logs.some((l) => l.includes("permissionSettled: empty requestId")),
  );
});
...
// ---- mcp-config 文件形态（token 不进 argv；写失败回落内联；清理时机）----

test("permission: prepareMcpConfig → argv 只出现路径（无 token）；进程退出清理该文件", async () => {
  const prepared: { port: number; token: string }[] = [];
  const cleaned: string[] = [];
  const deps = makeDeps({
    prepareMcpConfig: async (port, token) => {
      prepared.push({ port, token });
      return "/data/claudian/mcp-config-1.json";
    },
    cleanupMcpConfig: (path) => cleaned.push(path),
  });
  const bridge = createHostBridge(deps);
  const win = makeWin();
  handshake(deps, bridge, win);
  await tick();
  await sendAndSpawn(deps, bridge, win);

  assert.deepEqual(prepared, [{ port: 51234, token: MCP_TOKEN }]);
  const args = deps.turns[0].options.args;
  const i = args.indexOf("--mcp-config");
  assert.equal(args[i + 1], "/data/claudian/mcp-config-1.json");
  // 本改动的全部要点：argv 里任何元素都不得含该轮 token
  assert.ok(!args.some((a) => a.includes(MCP_TOKEN)));
  ...
  deps.turns[0].release();
  await tick();
  assert.deepEqual(cleaned, ["/data/claudian/mcp-config-1.json"]);
});

test("permission: prepareMcpConfig 返回 null（写失败）→ 回落内联 JSON，该轮照常发出", async () => {
  const deps = makeDeps({ prepareMcpConfig: async () => null });
  const bridge = createHostBridge(deps);
  const win = makeWin();
  handshake(deps, bridge, win);
  await tick();
  ...
  const args = deps.turns[0].options.args;
  const inline = args[args.indexOf("--mcp-config") + 1];
  assert.deepEqual(JSON.parse(inline), {
    mcpServers: {
      "claudian-perm": {
        type: "http",
        url: `http://127.0.0.1:51234/mcp?token=${MCP_TOKEN}`,
      },
    },
  });
});

test("permission: spawn 前失败（deny settings 写失败）→ 已写出的 mcp-config 文件同样清理，不 spawn", async () => {
  const cleaned: string[] = [];
  const deps = makeDeps({
    prepareMcpConfig: async () => "/data/claudian/mcp-config-2.json",
    cleanupMcpConfig: (path) => cleaned.push(path),
    prepareDenySettings: async () => {
      throw new Error("deny settings write failed: EACCES");
    },
  });
  const bridge = createHostBridge(deps);
  const win = makeWin();
...
test("permission: cleanupMcpConfig 抛错不掀翻解锁（清理是卫生动作）", async () => {
  const deps = makeDeps({
    prepareMcpConfig: async () => "/data/claudian/mcp-config-3.json",
    cleanupMcpConfig: () => {
  ...
  assert.ok(
    deps.logs.some((m) => m.includes("mcp config cleanup failed")),
    "清理失败要有日志",
  );
});
