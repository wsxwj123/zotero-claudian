...
//   A. newSessionId **两档**回退 + fail-closed
//      （R11 安全复查 2026-09-12 第二次点名：Math.random 那一档已删——它可预测，
//      两层都拿不到安全随机源时**抛错**，不再退回时间戳/弱随机 id）
...
/** 把 globalThis.crypto 换成 value，返回还原函数（异步用例自行 try/finally 调用） */
function swapCrypto(value: unknown): () => void {
...
function withCrypto<T>(value: unknown, fn: () => T): T {
  const restore = swapCrypto(value);
  try {
    return fn();
  } finally {
    restore();
...
test("A3: 连 WebCrypto 都没有 → fail-closed 抛错，绝不产出可预测 id（R11 安全复查：删弱随机回退）", () => {
  assert.throws(
    () => withCrypto(undefined, () => newSessionId()),
    /缺少安全随机源/,
    "无安全随机源必须抛错：旧实现退回「时间戳 + Math.random」，可预测且会拼进路径/参数位",
  );
});

test("A3b: fail-closed 落到 create —— 拒绝建会话，不落盘任何半成品记录", async () => {
  const { store, fs } = makeStore();
  const restore = swapCrypto(undefined);
  try {
    await assert.rejects(store.create({ title: "x" }), /缺少安全随机源/);
  } finally {
    restore();
  }
  assert.equal(store.list().length, 0, "内存里不得留下无 id 会话");
  assert.equal(
    fs.files.has(`${DATA_DIR}/sessions.json`),
    false,
    "索引不得落盘（否则重启后会出现一条读不了历史的幽灵会话）",
  );
});

test("A4: 两档产出都不含路径分隔符、不以 - 开头（拼路径/参数位安全）", () => {
  const stubs: unknown[] = [
  ...
  assert.equal(new Set(ids).size, ids.length, "两档产出应互不相同");
});
