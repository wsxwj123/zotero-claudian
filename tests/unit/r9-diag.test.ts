...
//
// 锁定的契约点（PLAN-R9）：
//   1) 报告 = 固定首行 + 14 行 `标签: 值`（标签补空格对齐，值在固定列开始）
...
test("R9-diag 报告：缺字段写 (error: 未采集)，采集失败写 (error: 原因)——整份报告仍产出", () => {
  ...
  assert.equal(lines.length, DIAG_KEYS.length, "缺字段不吞行：行数仍是 14");
...
test("R9-diag 报告：session/sessionFile/snapshots/journal 的空态写 (none)/false，不写假值", () => {
  ...
  assert.match(lines[11], /^snapshots: +dir=\(none\) count=0 lastTurn=none$/);
});
