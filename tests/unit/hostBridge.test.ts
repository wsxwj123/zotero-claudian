...
test("renameSession: 改名落索引 + 推 sessionList 回执；未知 id / 空标题 → 忽略不发消息", async () => {
  const deps = makeDeps();
  const bridge = createHostBridge(deps);
  const win = makeWin();
  helloHandshake(deps, bridge, win);
  ...
  bridge.dispatch({
    source: win,
    data: { type: "renameSession", sessionId, title: "  文献 A 精读  " },
  });
  await tick();
  assert.equal(deps.store.get(sessionId)?.title, "文献 A 精读"); // trim 后落库
  const list = lastMsg(deps, "sessionList");
  assert.equal(
    list?.type === "sessionList" ? list.sessions[0].title : null,
    "文献 A 精读",
  );
  assert.ok(deps.sent.length > before);

  // 未知 id / 空标题 → 忽略（不推列表、不改任何记录）
  const mark = deps.sent.length;
  bridge.dispatch({
    source: win,
    data: { type: "renameSession", sessionId: "nope", title: "x" },
  });
  bridge.dispatch({
    source: win,
    data: { type: "renameSession", sessionId, title: "   " },
  });
  await tick();
  assert.equal(deps.sent.length, mark);
  assert.equal(deps.store.get(sessionId)?.title, "文献 A 精读");
});
