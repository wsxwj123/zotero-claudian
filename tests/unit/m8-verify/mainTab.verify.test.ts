...
test("接线·register 注入按钮 → 点击走 openMainTab → unregister 关 tab + 摘按钮（插件停用）", () => {
  const doc = buttonDoc(["zotero-tabs-toolbar"]);
  const tabs = new FakeTabs(doc);
  const win: MainWindowLike = { document: doc, Zotero_Tabs: tabs };
  ...
  try {
    ...
    btn.fire("command");
    assert.equal(tabs.addCalls.length, 1, "点击按钮 → 建 tab");
...
test("接线·窗口卸载路径（带参）：摘该窗口按钮 + 只在该窗口承载 tab 时复位", () => {
  ...
  const otherWin: MainWindowLike = {
    document: other,
    Zotero_Tabs: new FakeTabs(other),
  };
  const g = globalThis as Record<string, unknown>;
  const savedAddon = g.addon;
  const savedZotero = g.Zotero;
  ...
  try {
    ...
    assert.equal(tabs.addCalls.length, 1);
    // 别的窗口卸载不该复位 tab
    unregisterMainTabEntry(otherWin);
