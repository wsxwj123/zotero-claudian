...
import { destroyDockForWindow } from "../../../src/modules/dockPanel.ts";
...
class FakeDoc implements DocumentLike {
  ...
  /** R5：dock 的父节点回落链末端（`getElementById("zotero-pane-stack") ?? documentElement`） */
  ...
  /** R5：dock 的 HTML 节点走 createElementNS（测试里与 XUL 节点同款 fake 即可） */
  createElementNS(_ns: string, tag: string): FakeNode {
    return this.createXULElement(tag);
  }
  ...
  createXULElement(tag: string): FakeNode {
    ...
    const node: FakeNode = {
      ...
      kids: [],
      parent: null,
      // R5：dock 布局会写 style.left/top/width/height（fake 只需可赋值）
...
/** R4-2：dock 链路要碰 Zotero.Prefs（dockWidth/dockOpen），给最小内存实现 */
function fakePrefs(): { store: Map<string, unknown>; Prefs: unknown } {
...
/** R4-2：能承载 dock 的 fake 主窗口（dock 布局读 innerWidth/Height，挂 resize 监听） */
function dockableWin(doc: FakeDoc, tabs: FakeTabs): MainWindowLike {
  return {
    document: doc,
    Zotero_Tabs: tabs,
    innerWidth: 1200,
...
test("接线·register 注入按钮 → 点击开关 dock（R4-2）→ unregister 关 tab + 摘按钮（插件停用）", () => {
  const doc = buttonDoc(["zotero-tabs-toolbar"]);
  const tabs = new FakeTabs(doc);
  const prefs = fakePrefs();
  const win = dockableWin(doc, tabs);
  ...
  try {
    ...
    // R4-2 入口改造：工具栏按钮不再开全页 tab，而是开关该窗口的右侧浮层（dock）
    btn.fire("command");
    assert.equal(tabs.addCalls.length, 0, "点击按钮不再建 tab（全页入口搬到 dock 头部）");
    assert.equal(
      prefs.store.get("extensions.zotero-claudian.dockOpen"),
      true,
      "点击按钮应打开 dock（dockOpen 落 prefs）",
    );
    // 再点一次 = 关（toggle 幂等语义）
    btn.fire("command");
    assert.equal(
    ...
    unregisterMainTabEntry();
    destroyDockForWindow(win as unknown as Window); // dock 是模块级 Map，测试间要收干净
    g.addon = savedAddon;
...
test("接线·窗口卸载路径（带参）：摘该窗口按钮 + 只在该窗口承载 tab 时复位", () => {
  ...
  const otherWin = dockableWin(other, new FakeTabs(other));
  const g = globalThis as Record<string, unknown>;
  const savedAddon = g.addon;
  const savedZotero = g.Zotero;
  const savedServices = g.Services;
  ...
  try {
    ...
    // R4-2：按钮开关 dock，不再建 tab
    doc.getElementById(TOOLBAR_BUTTON_ID)!.fire("command");
    assert.equal(tabs.addCalls.length, 0, "按钮只开关 dock");
    assert.equal(
      prefs.store.get("extensions.zotero-claudian.dockOpen"),
      true,
      "点击按钮应打开 dock",
    ...
    unregisterMainTabEntry();
    destroyDockForWindow(win as unknown as Window);
    destroyDockForWindow(otherWin as unknown as Window);
    g.addon = savedAddon;
    g.Zotero = savedZotero;
    g.Services = savedServices;
  }
