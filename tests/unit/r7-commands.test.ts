...
//
// 锁定的契约点：
//   1) 本地命令 = 硬编码白名单 9 个（/new /clear /note /instructions /workspace /balance /export /help /diag），
...
/** 本地命令白名单的期望名（PLAN §3.5 表格逐字 + R9 增补 /diag） */
const LOCAL_NAMES = [
  "new",
  "clear",
  "note",
...
test("R7-C 白名单：九个本地命令齐全（/new /clear /note /instructions /workspace /balance /export /help /diag）", () => {
...
test("R7-C 白名单：名称 → 动作一一对应（九个动作互不相同，不串台）", () => {
