...
//
// R11 API 收敛：返回形状改为 {found, capped}（原本是「path 对象或 null」）——语义逐条不变，
// 只把「扫完了没有」并进同一个返回；断言未放宽（找不到即 found:null，找到即给正确路径）。
//
...
//   3) 都找不到 → found:null（调用方 fail-safe：按钮禁用/拒绝回滚，绝不猜路径）
//   4) id 必须过 sessionStore 白名单：非法 id 直接 found:null 且**一个 fs 调用都不发**（防 `../` 注入）
//   5) 扫描有上限（PROJECTS_SCAN_LIMIT=500）：超限只扫前 500 个目录，仍未命中则 log +
//      found:null,capped:true（「没扫完」必须与「确实没有」区分得开）
...
test("R8-SESSION-FILE 长路径（cwd 原串与 CLI 真名都超 200 截断）也能命中", async () => {
  ...
  assert.equal(found.found?.projectDir, real);
  assert.equal(found.found?.path, `${ROOT}/${real}/${ID}.jsonl`);
});

test("R8-SESSION-FILE 都找不到 → found:null（调用方 fail-safe，不猜）", async () => {
  const { fs, calls } = fakeFs([], { [ROOT]: ["-Users-me-a", "-Users-me-b"] });
  const found = await findClaudeSessionFile(
    { projectsRoot: ROOT, claudeSessionId: ID, cwd: WS_LINK },
    { fs },
  );
  assert.deepEqual(found, { found: null, capped: false });
  assert.deepEqual(calls.listNames, [ROOT]);
  // 探测的都是「本 id 在该目录下」的路径，不会拿别人的会话文件冒充
  assert.ok(calls.exists.every((p) => p.endsWith(`/${ID}.jsonl`)));
});

test("R8-SESSION-FILE 目录里只有别人的同名会话 id 不同 → found:null", async () => {
...
test("R8-SESSION-FILE 非法 id → 直接 found:null 且一个 fs 调用都不发（防 `../` 注入）", async () => {
  const bad = [
    "",
    "..",
    ".",
  ...
  for (const id of bad) {
    ...
    assert.deepEqual(
      found,
      { found: null, capped: false },
      `非法 id 必须拒绝：${JSON.stringify(id)}`,
    );
...
test("R8-SESSION-FILE 扫描上限：超限只扫前 500 个目录，命中算命中", async () => {
  ...
  assert.equal(found.found?.projectDir, target);
  assert.equal(found.capped, false, "命中即停，与目录总数无关");
  // 快路径 1 次 + 扫描 ≤ 上限次（命中即停）
  assert.ok(
...
test("R8-SESSION-FILE 扫描上限：目标在限外 → found:null + capped:true + 留 log，且不越界多探", async () => {
  const names = Array.from(
    { length: PROJECTS_SCAN_LIMIT + 20 },
    (_, i) => `-dir-${i}`,
  );
  ...
  assert.deepEqual(found, { found: null, capped: true });
...
test("R8-SESSION-FILE 目录名异常（带分隔符/上跳）不参与拼接", async () => {
  ...
  assert.equal(found.found, null, "含分隔符/上跳的目录名不参与拼接");
  assert.ok(
...
test("R8-SESSION-FILE 扫目录抛错 → found:null（不把异常抛给调用方）", async () => {
...
test("R8-SESSION-FILE 快路径与慢路径都命中时用快路径（不扫、结果稳定）", async () => {
  ...
  assert.equal(found.found?.projectDir, derived);
