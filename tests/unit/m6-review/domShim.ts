...
export class ShimElement {
  ...
  /**
   * 选择器只实现渲染后处理真正用到的最小子集：`tag` / `.class` / `tag.class`（逗号可并列）。
   * 真 DOM 的复合/层级/属性选择器在这层不实现——用例要的是「渲染产物里有没有这个块」，
   * 复杂选择器请用 queryAll/byClass 自写谓词。返回数组（够 forEach/length 用）。
   */
  querySelectorAll(selector: string): ShimElement[] {
    const out: ShimElement[] = [];
    for (const part of String(selector).split(",")) {
      const m = /^([a-zA-Z][\w-]*)?(?:\.([\w-]+))?$/.exec(part.trim());
      if (!m || (!m[1] && !m[2])) {
        continue;
      }
      const [, tag, cls] = m;
      for (const el of queryAll(this, (e) => matchSimple(e, tag, cls))) {
        if (!out.includes(el)) {
          out.push(el);
        }
      }
...
/** querySelectorAll 的最小选择器匹配：tag 与 class 都给出时需同时满足 */
function matchSimple(
  el: ShimElement,
  tag: string | undefined,
  cls: string | undefined,
): boolean {
  if (tag && el.localName !== tag.toLowerCase()) {
    return false;
  }
  if (cls && !el.className.split(/\s+/).filter(Boolean).includes(cls)) {
    return false;
  }
  return true;
}
