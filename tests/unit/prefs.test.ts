...
// ---- R5：权限档校验（设置页/新会话默认档；纯函数，node 直测）----

test("prefs: normalizePermissionMode 接受四档（含新增 bypass 放任档）", () => {
  for (const ok of ["default", "acceptEdits", "plan", "bypass"] as const) {
    assert.equal(normalizePermissionMode(ok), ok);
  }
});

test("prefs: normalizePermissionMode 非法值/脏值 → 回落 acceptEdits", () => {
  for (const bad of ["bypassPermissions", "", "PLAN", null, undefined, 42]) {
    assert.equal(normalizePermissionMode(bad), "acceptEdits", String(bad));
  }
});
