...
// ---------- R4 复查修-1：空绑定发送 → 先建会话、绑定后再发原文 ----------
// 症状（复查证据）：followReader 绑 null 后 sessionId=null，旧路径把 {type:'send', sessionId:null}
// 交给宿主 → 宿主 resolveSendTarget(null) 落到「最近会话」并按本轮 itemKey 改写它的归属：
// 消息进别的文献的会话、原归属被静默改绑。修法：UI 侧先走 createSession，原文暂存 queuedSend，
// 绑定建立后由 flushQueuedSend 恰发一次。

/** readerContext 最小形态（宿主推送时四字段齐全） */
const ctx = (itemKey: string | null): HostMessage => ({
  type: "readerContext",
  itemKey,
  title: "某文献",
  page: 1,
...
test("复查修-1: 空绑定发送 → createSession（带当前 itemKey），绑定后原文恰发一次；不产生 send(null)", () => {
  const start: ChatState = {
    ...initialChatState(),
    connected: true,
    readerContext: { itemKey: "ITEM_B", title: "B", page: 1, selection: null },
  };
  const r = sendWithAutoSession(start, "这篇讲了什么？");
  assert.deepEqual(
    r.msg,
    { type: "createSession", itemKey: "ITEM_B" },
    "空绑定发送没有先建会话（或 itemKey 没带当前文献）",
  );
  assert.equal(r.state.creatingSession, true);
  assert.equal(r.state.queuedSend, "这篇讲了什么？", "原文没有暂存（会丢）");

  // 绑定建立前：不产生任何 send（尤其不能带 null sessionId）
  assert.notEqual(r.msg?.type, "send");

  // 宿主回列表（新会话属于当前文献）→ 绑定 + 暂存原文原样保留
  const bound = feed(r.state, {
    type: "sessionList",
    sessions: [summary("S-new", { itemKey: "ITEM_B" })],
  });
  assert.equal(bound.sessionId, "S-new");
  assert.equal(bound.queuedSend, "这篇讲了什么？");

  // 绑定一到 → 发原文（用绑定后的 sessionId），并落成视图里的乐观轮
  const f = flushQueuedSend(bound);
  assert.deepEqual(f.msg, {
    type: "send",
    sessionId: "S-new",
    text: "这篇讲了什么？",
  });
  assert.equal(f.state.queuedSend, null);
  assert.deepEqual(
    f.state.messages.map((m) => m.text),
    ["这篇讲了什么？"],
  );

  // 恰一次：再 flush 不重发、不追加轮
  const again = flushQueuedSend(f.state);
  assert.deepEqual(again.msg, null, "原文被重复发送");
  assert.equal(again.state.messages.length, 1);
});

test("复查修-1: 无 readerContext（通用会话）同样先建会话；itemKey=null", () => {
  const r = sendWithAutoSession(initialChatState(), "通用问题");
  assert.deepEqual(r.msg, { type: "createSession", itemKey: null });
  const bound = feed(r.state, {
    type: "sessionList",
    sessions: [summary("S-gen")],
  });
  assert.equal(bound.sessionId, "S-gen");
  assert.deepEqual(flushQueuedSend(bound).msg, {
    type: "send",
    sessionId: "S-gen",
    text: "通用问题",
  });
...
test("复查修-1: 建会话失败（error）→ 原文还给输入框 + 横幅；此后不再自动发", () => {
  const start: ChatState = {
    ...initialChatState(),
    connected: true,
    readerContext: { itemKey: "ITEM_B", title: "B", page: 1, selection: null },
  };
  const r = sendWithAutoSession(start, "问题");
  const failed = feed(r.state, {
    type: "error",
    code: "ITEM_NOT_FOUND",
    message: "条目不存在",
  });
  assert.equal(failed.queuedSend, null);
  assert.equal(
    failed.restoreDraft,
    "问题",
    "原文既没发出也没还给输入框（丢了）",
  );
  assert.ok(failed.errorBanner?.startsWith("ITEM_NOT_FOUND"));
  assert.equal(failed.creatingSession, false);

  // 之后即便绑定出现，也不会把原文补发出去（已在输入框里，由用户决定）
  const later = feed(failed, {
    type: "sessionList",
    sessions: [summary("S-x", { itemKey: "ITEM_B" })],
  });
  assert.deepEqual(flushQueuedSend(later).msg, null);
});

test("复查修-1: 已绑定会话 → 与 userSend 同路径（不发 createSession、不暂存）", () => {
  const s = feed(initialChatState(), {
    type: "sessionList",
    sessions: [summary("S1")],
  });
  const r = sendWithAutoSession(s, "正常发送");
  assert.deepEqual(r.msg, { type: "send", sessionId: "S1", text: "正常发送" });
  assert.equal(r.state.creatingSession, false);
  assert.equal(r.state.queuedSend, null);
});

test("复查修-1: 已绑定但 turn 进行中 → 无动作（并发契约不变）", () => {
  let s = feed(initialChatState(), {
    type: "sessionList",
    sessions: [summary("S1")],
  });
  s = userSend(s, "在途").state; // turnStatus = waiting
  const r = sendWithAutoSession(s, "不排队");
  assert.equal(r.msg, null);
  assert.equal(r.state, s);
});

// ---------- R4 复查修-2：绑定必须尊重当前文献（两种到达顺序收敛到一处） ----------

const twoItems = [
  summary("SA", { itemKey: "ITEM_A", updatedAt: 9000 }),
  summary("SB", { itemKey: "ITEM_B", updatedAt: 1000 }),
];

test("复查修-2: readerContext 先到（sessions 还空）+ sessionList 后到 → 绑当前文献的会话", () => {
  const s = feed(initialChatState(), ctx("ITEM_B"), {
    type: "sessionList",
    sessions: twoItems,
  });
  assert.equal(
    s.sessionId,
    "SB",
    "未绑定 → 取了全量最新（别文献的会话），且此后不会自我纠正",
  );
  assert.equal(s.sessions.find((x) => x.id === s.sessionId)?.itemKey, "ITEM_B");
});

test("复查修-2: sessionList 先到 + readerContext 后到 → 同样绑当前文献的会话", () => {
  const s = feed(
    initialChatState(),
    { type: "sessionList", sessions: twoItems },
    ctx("ITEM_B"),
  );
  assert.equal(s.sessionId, "SB");
});

test("复查修-2: 列表刷新（别文献有更新的会话）不抢走当前文献的绑定", () => {
  let s = feed(initialChatState(), ctx("ITEM_B"), {
    type: "sessionList",
    sessions: twoItems,
  });
  s = feed(s, {
    type: "sessionList",
    sessions: [
      summary("SA", { itemKey: "ITEM_A", updatedAt: 99999 }),
      ...twoItems,
    ],
  });
  assert.equal(s.sessionId, "SB");
...
test("复查修-2: 当前文献没有会话 → 解绑而不是落到别文献的会话", () => {
  const s = feed(initialChatState(), ctx("ITEM_C"), {
    type: "sessionList",
    sessions: twoItems,
  });
...
test("复查修-2: 例外保留——无会话时 send 自动建的会话（在途乐观轮）不被收敛清掉", () => {
  // handleSend 自动建会话：UI 未绑定、已有乐观 user 轮（waiting），列表回来时新会话可能
  // 还没带上 itemKey（宿主先推列表、后随本轮上下文补 itemKey）
  const inFlight: ChatState = {
    ...initialChatState(),
    sessionId: null,
    turnStatus: "waiting",
    messages: [{ role: "user", text: "在途问题" }],
    readerContext: { itemKey: "ITEM_B", title: "B", page: 1, selection: null },
  };
  const s = feed(inFlight, {
    type: "sessionList",
    sessions: [summary("S-auto", { itemKey: null })],
  });
  assert.equal(s.sessionId, "S-auto");
  assert.deepEqual(
    s.messages.map((m) => m.text),
    ["在途问题"],
    "自动建会话的在途乐观轮被跟随收敛清掉了（既有例外失效）",
  );
});
