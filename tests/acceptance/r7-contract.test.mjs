...
//   §3.8 消息级操作：COLLAPSE_LINE_THRESHOLD(12) / CODE_COLLAPSE_LINE_THRESHOLD(20) —— 纯 UI 面，
//     无新协议消息（其余用例在 tests/unit/r7-message-actions.test.mjs、R8 口径在 tests/unit/r8-collapse.test.mjs）
...
test('R7 §3.5：本地命令白名单是枚举 —— 九个命令 + 未知命令零执行通道', () => {
...
// R8 折叠口径按用户反馈收窄（原话：「应该是**代码块包裹的内容**自动折叠，**用户的消息超过一定长度后**
// 自动折叠，不是所有内容都折叠」）：12 行只对**用户消息**（整条折）；AI 回复**正文恒不折**，
// 回复里的**代码块**按 20 行（CODE_COLLAPSE_LINE_THRESHOLD）按块折。

test('R7 §3.8：折叠阈值 12 行（用户消息；超过才默认折叠）+ R8 代码块 20 行', () => {
