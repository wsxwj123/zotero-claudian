...
//   normalizeDockWidth(raw) / dockRect(viewport, raw) / dockReduce(phase, action)：右侧浮层几何与开合
...
test('R4 §3：宽度钳制 —— 非有限/非数字/<=0 回落默认，越界钳到边界', () => {
  assert.equal(DOCK_WIDTH_DEFAULT, 420);
  assert.equal(normalizeDockWidth(undefined), DOCK_WIDTH_DEFAULT);
  assert.equal(normalizeDockWidth('550'), DOCK_WIDTH_DEFAULT);
  assert.equal(normalizeDockWidth(Number.NaN), DOCK_WIDTH_DEFAULT);
...
test('R4 §3：几何 —— 贴右缘、上下贴合内容区、宽度=钳制后宽度', () => {
  const vp = { left: 0, top: 37, width: 1440, height: 963 };
  const rect = dockRect(vp, 420);
  assert.equal(rect.width, 420);
  assert.equal(rect.left + rect.width, vp.left + vp.width);
  assert.equal(rect.top, 37);
  assert.ok(rect.height > 0);
});

test('R4 §3：开合状态机 —— 两次 toggle 回到原态，destroy 后 toggle 不复活', () => {
  ...
  assert.equal(dockReduce(dead, 'toggle'), 'destroyed');
});
