...
- **多个会话同时跑很吃内存**：每个正在进行的会话都是一个独立的 Claude Code 进程，实测常驻约 **300MB**（还没算它拉起的工具子进程）——同时跑 3 个会话 ≈ 1GB。**建议同时最多跑 1–2 个**；界面发钝主要来自多个进程抢内存；长回答的渲染离屏实测（约 100KB 一次约 18ms）不是主因——v0.1.29 加的渲染耗时埋点在 macOS 真机冒烟里实测单次 paint 最大 2.9ms、慢渲染 0 次，Windows 真机数据待补
- LaTeX 公式渲染 v1 不支持（原文保留，v2 计划）；**mermaid 图与 SVG 已支持渲染**（HTML 保持源码显示——安全考虑）
- **「范围」一次最多带 20 篇进提示词**：范围按钮本身允许选到 40 篇，但发送时按 20 篇截断，第 21 篇起不进本轮上下文（下一版修）
...
- **改 `~/.claude/settings.json` 下一轮对话就生效；改 shell / 系统环境变量必须重启 Zotero**——插件拉起 CLI 时原样透传 Zotero 进程的环境变量（只覆盖 PATH），已经在跑的 Zotero 拿的还是它启动那一刻的旧环境。同理，你在终端 `export` 的 `https_proxy` 面板里也拿不到；需要代理才能连上模型服务商时，要让 Zotero 自己带着代理环境启动。

**从第三方中转切回官方订阅，报 `Failed to authenticate: OAuth session expired and could not be refreshed`？** 这是 `claude` CLI 自己的登录态失效，插件只是把 CLI 的报错原样显示出来。用中转期间走的是 `ANTHROPIC_AUTH_TOKEN`，官方 OAuth 会话一直没被使用，刷新令牌到期后就换不回去了（刷新地址是 CLI 写死的官方端点，跟 `ANTHROPIC_BASE_URL` 无关）。处理：在终端跑 `claude` 用 `/login` 重新登录 → 确认中转的两个变量在 settings、系统环境变量、项目级 `.claude/settings.json` 三处都清干净 → 重启 Zotero。如果报的是「another Claude Code process is refreshing it」那是瞬态的，等一分钟重试即可。
