...
/** 每会话条数上限（超出丢最老；与页面 INPUT_HISTORY_LIMIT 同值，宿主侧是第二道闸） */
export const INPUT_HISTORY_LIMIT = 50;
/** 写盘合并窗口：窗口内多次 save 只落一次盘 */
export const DEFAULT_COALESCE_MS = 500;

/**
 * 条目归一（读写两侧共用）：只留非空字符串、相邻重复只留一份、超出丢最老。
 * 与页面 lib/inputHistory.ts 的归一/recordSent 同口径——落盘数据被外部改坏时按同一套规则收敛。
 */
...
/**
 * 并入（而非替换）该会话已存的历史：页面递来的那份**通常是**「宿主那份 + 本页面新发的几条」，
 * 但也可能是一个刚重载、还没收到宿主历史的页面递来的残份，或另一个面板实例的本地副本——
 * 直接替换会把这期间别处发的条目、以及重载前的旧条目整段抹掉（历史只增不减，BUG-21 同款教训）。
 * 按「已存的是新来者的前缀」对齐后拼接；重叠加尾部的上限裁剪会把重复段收敛掉。
 */
export function mergeHistoryEntries(
  ...
  return normalizeHistoryEntries([...stored, ...incoming.slice(i)]);
}
