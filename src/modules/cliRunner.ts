...
export function buildSpawnArgs(opts: BuildSpawnArgsOptions): string[] {
  ...
  if (opts.resumeClaudeSessionId) {
    // 信任边界（安全复查 2026-09-12）：id 进 argv 前必须再校验一次——否则 `--foo` 形态会被
    // CLI 当选项解析（flag smuggling），而这条路径的调用方分布在多处（首轮/续接/分叉）。
    // 判据与 sessionStore 共用 utils/ids，非法即抛（调用方回 SPAWN_FAILED），不静默纠正。
    if (!isSafeSessionId(opts.resumeClaudeSessionId)) {
      throw new Error(
        `invalid resumeClaudeSessionId: ${String(opts.resumeClaudeSessionId)}`,
      );
    }
...
import { isSafeSessionId } from "../utils/ids";
