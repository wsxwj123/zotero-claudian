 ...
 * 返回实际命中的目录名 + 路径，并带「扫完了没有」（`capped`，`/diag` 要它）；
 * 找不到 → `found=null`（调用方 fail-safe）。回滚/分支那条路只用得到 `.found`。
 */
async function claudeSessionLookupForHost(
  cwd: string,
  claudeSessionId: string,
): Promise<SessionFileLookup> {
  return lookupClaudeSessionFile(
...
function getHostBridge(): HostBridge {
  ...
  const deps: HostBridgeDeps = {
    ...
    rewind: {
      dataDir: sessionDataDir(),
      fs: rewindFs,
      // 契约只要命中结果（`capped` 是 /diag 的读者需要的东西，走别的路）
      findSessionFile: async (cwd, claudeSessionId) =>
        (await claudeSessionLookupForHost(cwd, claudeSessionId)).found,
