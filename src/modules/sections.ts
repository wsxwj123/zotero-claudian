...
import { buildDiagReport, ZOTERO_SUPPORT_RANGE } from "../utils/diag";
import {
...
async function runProbe(
  ...
  try {
    ...
    exited = true;
    await readDone;
    return { ok: status.exitCode === 0, stdout, timedOut };
  } catch (err) {
...
/** R7-C：命令清单（宿主只读 frontmatter；扫描异常在纯逻辑里收敛成空清单） */
async function listCommandsFromHost(): Promise<CommandEntry[]> {
  return scanCommands(commandScanDeps());
}
 ...
 * 「当前分类全部」，故 id 由宿主现取，UI 永不传 id）；无选中分类 → 空清单（UI 显示 0 篇）。
 */
 ...
 * claudeSessionFileForHost（按会话 id 定位）。
 */
 ...
 * 返回实际命中的目录名 + 路径；找不到 → null（调用方 fail-safe）。
 */
async function claudeSessionFileForHost(
  cwd: string,
...
//  - 绝不输出密钥/令牌：报告行由 utils/diag 白名单定死，本文件的采集器**只传**这 14 项；
//  - 不得触发权限卡 / 不得 spawn CLI 轮次（唯一子进程 = `claude --version`，3s 超时 → "timeout"）；
//  - 不得改任何状态：不建目录、不写索引、不碰 CLI 会话文件（全部只读）；
//  - 降级不抛：每项独立兜底，失败写 `(error: 原因)`，整份报告照出。

/** `claude --version` 探测超时（诊断专用，比启动检测的 10s 短：弹层不该干等） */
const DIAG_VERSION_TIMEOUT_MS = 3000;
...
async function diagCliLine(): Promise<string> {
  ...
  const version = run.timedOut
    ...
    : run.ok && firstLine
      ? firstLine
      : "(error: 无法执行)";
  return `command=${base.command} channel=${base.channel} version=${version}`;
}
...
async function diagWorkspaceLine(): Promise<string> {
  ...
  const writable = exists ? (await IOUtils.getFile(root)).isWritable() : false;
...
async function diagSessionFileLine(
  ...
  const found = await claudeSessionFileForHost(cwd, claudeSessionId);
  return found
    ? `found=true via=scan dir=${found.projectDir}`
    : "found=false via=none dir=(none)";
}
...
function getHostBridge(): HostBridge {
  ...
  const deps: HostBridgeDeps = {
    ...
    rewind: {
      dataDir: sessionDataDir(),
      fs: rewindFs,
      findSessionFile: claudeSessionFileForHost,
