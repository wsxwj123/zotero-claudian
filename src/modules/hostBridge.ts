...
export function createHostBridge(deps: HostBridgeDeps): HostBridge {
  ...
  async function handleSend(
    ...
    try {
      ...
      deps.log(
        `[bridge] spawning turn: session=${sessionId} resume=${record.claudeSessionId ?? "none"} addDir=${input.addDir ?? "none"} denySettings=${denySettings ?? "none"} mcpConfig=${mcpConfigPath ?? "inline"} cwd=${workspace}`,
      );
