...
import { SCOPE_ITEMS_MAX } from "../utils/scope";
...
export interface HostBridgeDeps {
  ...
  rewind?: {
     ...
     * cwd 推目录名，未命中扫 `<~/.claude>` 的 projects 根一层）。返回 null = 找不到 → 调用方
     * 走 fail-safe（快照落空、回滚拒绝）。
 ...
 * 真宿主给 findSessionFile（按会话 id 扫 projects 一层 → 符号链接/长路径工作区也命中）；
 * 老宿主/测试只有 cwd 推导两条 → 退回它们（行为逐字不变）。定位失败（含抛错）→ null，
 * 调用方按既有 fail-safe 处理（不猜路径、不拿 cwd 现推去当权威）。
 */
async function locateSessionFile(
  ...
  if (cfg.findSessionFile) {
    try {
      return await cfg.findSessionFile(workspace, claudeSessionId);
    } catch (err) {
...
export function createHostBridge(deps: HostBridgeDeps): HostBridge {
  ...
  async function handleListCommands(win: UiWindowKey): Promise<void> {
    ...
    sendTo(win, {
      type: "commandList",
      commands: [...LOCAL_COMMANDS, ...scanned],
    });
  }
  ...
  /** R7-D：范围解析（kind 枚举收口；未接线 → 空清单回执，UI 显示 0 篇） */
  async function handleResolveScope(
    ...
    let res: ResolvedScope = {
      kind,
      label: kind === "collection" ? "当前分类" : "选中条目",
      items: [],
      truncated: false,
    };
    try {
