...
export interface HostBridgeDeps {
  ...
  rewind?: {
    ...
    dataDir: string;
    fs: RewindFs;
    /** cwd → CLI 项目目录名（`~/.claude/projects/<此值>/`，utils/rewind.encodeProjectDir） */
    projectDirFor(cwd: string): string;
    /** CLI 自己的会话文件绝对路径（分叉前要被替换的那份） */
...
export function createHostBridge(deps: HostBridgeDeps): HostBridge {
  ...
  /**
   * R7-H：本轮结束拍快照（原文件逐字节 → `snapshots/<会话id>/<max+1>.jsonl`）。
      ...
      await snapshotTurn(
        {
          ...
          sourcePath: cfg.claudeSessionPath(workspace, cli),
        },
        { fs: cfg.fs },
      );
    } catch (err) {
  ...
  async function handleSend(
    ...
    try {
      ...
      if (forkReady && deps.rewind && fork && forkParentCli) {
        const cfg = deps.rewind;
        const result = await rewindToTurn(
          {
            dataDir: cfg.dataDir,
            ...
            turn: fork!.turn,
            projectDir: cfg.projectDirFor(workspace),
            sourcePath: cfg.claudeSessionPath(workspace, forkParentCli),
            cwd: workspace,
          },
          {
            fs: cfg.fs,
