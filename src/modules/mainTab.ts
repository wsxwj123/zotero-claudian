...
import { activateChatPane, type PaneDocumentLike } from "./paneAutoShow";
...
export interface ZoteroTabsLike {
  ...
  /** 当前选中的标签类型（R8：决定去 context-pane 还是 item-pane 找 Claude 面板；老版本可能没有） */
  selectedType?: string;
}
...
/**
 * 当前选中的标签类型。取不到（老版本没有 selectedType）按 "library" 处理——
 * 书库是默认标签，指向那个此刻可见的侧栏；猜错只会落到「找不到宿主」的日志，不会误点。
 */
function selectedTabType(win: MainWindowLike): string {
  try {
    const type = win.Zotero_Tabs?.selectedType;
    return typeof type === "string" && type ? type : "library";
  } catch (err) {
    Zotero.logError(err as Error);
    return "library";
  }
...
/**
 * 工具栏按钮入口（R8）：展开该窗口右侧栏 + 定位到 Claude 面板（读者的 PDF 自动让位，不是浮层遮盖）。
 * 逻辑全在 paneAutoShow.activateChatPane（与「打开文献自动切面板」共用同一套三步），这里只做接线。
 */
export function showChatPane(win: MainWindowLike): void {
  activateChatPane(
    {
      // MainWindowLike.document 只声明了本模块用到的成员：这里按实际文档面断言
      doc: win.document as unknown as PaneDocumentLike,
      paneID: CHAT_PANE_ID,
      tabType: selectedTabType(win),
    },
    { log: (message) => Zotero.debug(`[claudian] ${message}`) },
  );
}
 ...
 * R8：按钮行为 = 打开右侧栏里的 Claude 面板（R4 的浮层方案整体删除）。
 */
