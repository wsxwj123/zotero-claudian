...
function wirePrefsPane(win: Window): void {
  ...
  // R6：工作区模式（单一目录 / 按合集分目录）
  setText(byId("ws-mode-label"), getString("prefs-ws-mode-label"));
  setText(byId("ws-mode-single"), getString("prefs-ws-mode-item-single"));
  setText(
    byId("ws-mode-collection"),
    getString("prefs-ws-mode-item-collection"),
  );
  setText(byId("ws-mode-hint"), getString("prefs-ws-mode-hint"));
