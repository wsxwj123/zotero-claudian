...
export interface AttachmentInfo {
  ...
  pdfDir: string | null;
  /** 父条目 id；null = 独立附件（无父条目 → 通用会话，整块省略） */
...
export interface TurnContext {
  /** 会话绑定条目 key（父条目）；null = 通用会话（上下文块整块省略） */
  itemKey: string | null;
 ...
 * reader → 附件 → 父条目元数据，任一环缺失即退化为通用会话（整块省略）。
...
export async function buildTurnContext(
  ...
  const parent =
    parentItemID == null ? null : await deps.getItemMetadata(parentItemID);
  ...
  const promptContext: PromptContext = {
    itemKey: parent.key,
    displayTitle: parent.displayTitle,
    creators: parent.creatorNames,
    date: parent.date,
    doi: parent.doi,
