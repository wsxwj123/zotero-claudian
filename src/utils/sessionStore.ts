...
export interface SessionStoreFs {
   ...
   */
  writeText(path: string, data: string, opts?: { mode?: number }): Promise<void>;
  /** 追加写（文件不存在则创建）。历史文件只走这个口——既有内容绝不重写（BUG-21） */
  appendText(path: string, data: string, opts?: { mode?: number }): Promise<void>;
  /** 同目录改名/移动，目标已存在时覆盖 */
...
/**
 * 会话 id 生成（三档回退，三档都过 ID_PATTERN）：
 *   ① crypto.randomUUID()（uuid）—— 正常环境恒走这档；
 *   ② crypto.getRandomValues 自造 hex —— 有 WebCrypto 但没 randomUUID 的旧 Gecko/测试桩；
 *   ③ 时间戳串 —— **只有连 WebCrypto 都没有的环境**才走到（仅测试环境可能出现）。
 * 回退档不再用 Math.random()：它可预测（V8 的 xorshift128+ 可由少量输出反推状态），
 * 会话 id 会拼进路径与 `--resume` 参数位，不该拿弱随机当标识。
 */
export function newSessionId(): string {
  ...
  try {
    ...
    // 连 WebCrypto 都没有 → 第三档兜底
  }
  // ponytail: 仅测试环境可能走到这里；时间戳形态可预测，非安全用途（别当令牌/别当唯一性保证）
