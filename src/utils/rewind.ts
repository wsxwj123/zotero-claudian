 ...
 *
 * 返回 `{found, capped}` 而不是只给命中的那条：`/diag` 必须能区分「扫完了、确实没有」与
 * 「没扫完、被上限挡了」；回滚/分支/快照的调用方只用得上 `.found`（形状统一，R11 API 收敛）。
 */
export async function findClaudeSessionFile(
  ...
  try {
    ...
    deps.log?.(
      `findClaudeSessionFile: list ${input.projectsRoot} failed: ${String(err)}`,
    );
    return { found: null, capped: false };
  }
  ...
  if (capped) {
    deps.log?.(
      `findClaudeSessionFile: ${sorted.length} dirs under ${input.projectsRoot} > ${PROJECTS_SCAN_LIMIT}, scan capped`,
    );
  }
