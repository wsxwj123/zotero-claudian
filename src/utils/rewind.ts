...
//
// 信任边界：原文件路径（sourcePath）由宿主**按会话 id 定位**（findClaudeSessionFile，R8）——
// 不再拿 cwd 现推目录名（CLI 用的是物理路径，符号链接/长路径下推不准）；本模块只认
// 「快照索引里记的 projectDir 与实际命中的目录一致」这一条闸——不一致即拒绝（工作区变了，绝不猜）。

import { isSafeId } from "./sessionStore";
...
export interface RewindFs {
  ...
  /**
   * 可选能力（**未提供时回落到按目录名排序**）：目录/文件的修改时间（ms）。
   * 存在的理由：`~/.claude/projects` 目录数超过扫描上限时，"最近写过"的项目目录
   * 更可能是目标 —— 按 mtime 降序能把命中率显著抬高（真宿主已提供）。
   * 取不到（不存在/异常）→ null，调用方按"垫底"处理。
   */
 ...
 * **只作快路径提示**（findClaudeSessionFile 的第一步试它），权威来源是按会话 id 定位——
 * CLI 派生的名字基于它自己看到的 **物理路径**（Node `process.cwd()`，符号链接已解析），
 * 拿配置里的原串转义在符号链接/长路径下会推错，故本函数的结果**不可**当唯一真相。
...
/** 慢路径扫描的目录数上限（projectsRoot 下一层；超限只扫前这么多，扫不到就放弃） */
export const PROJECTS_SCAN_LIMIT = 500;

export interface FindSessionFileInput {
  /** CLI 的项目目录根（`~/.claude/projects`） */
  projectsRoot: string;
  /** CLI 会话 id（必须过 sessionStore 的 id 白名单；不合形态直接 null，不碰 fs） */
  claudeSessionId: string;
  /** 本轮工作区原串（可能含符号链接）：**只**用于推快路径目录名 */
  cwd: string;
}

export interface FoundSessionFile {
  /** `<projectsRoot>/<实际目录名>/<id>.jsonl`：原样返回（不 realpath——读写要用同一拼法） */
  path: string;
  /** 实际命中的目录名（PROJECT_DIR_MISMATCH 闸门比对的就是它） */
  projectDir: string;
}

/**
 * 慢路径扫描顺序：**自然序**（数字段按数值比：`-dir-3` < `-dir-10` < `-dir-519`）。
 * 默认的 code-unit 序会把 `-dir-519` 排到 `-dir-3` 前面（与「项目目录越新越靠后」的直觉相反，
 * 也让「目标在限外」变得难复现）；Intl 的 numeric 序在目标引擎（Gecko ≥115 / Node）里都在且确定。
 */
const scanOrder = new Intl.Collator("en", { numeric: true }).compare;

export interface SessionFileLookup {
  /** 命中就是它；未命中 = null（调用方维持既有 fail-safe） */
  found: FoundSessionFile | null;
  /** **没扫完**的标志：目录数超上限、只扫了前 PROJECTS_SCAN_LIMIT 个仍未命中 → true。
   *  false 才代表「该扫的都扫了，确实没有」；/diag 靠它区分「找不到」与「没找完」。 */
  capped: boolean;
}

/**
 * CLI 会话文件定位（R8）：按 `<claudeSessionId>.jsonl` 找文件，**不猜目录名**。
 *
 * 为什么不复刻 encodeProjectDir 的目录名派生：CLI 用的是它自己看到的 cwd，而 Node 的
 * `process.cwd()` 给的是**物理路径**（macOS `/tmp`→`/private/tmp`；Windows junction/8.3 短名同理），
 * 配置里的工作区原串却是符号链接路径 → 转义串、>200 截断与哈希全跟着变（真机实测：工作区
 * `/tmp/xxx` 时 CLI 落在 `-private-tmp-…`，我们推的是 `-tmp-…`）。要在 Gecko 侧复刻 Node 的
 * 物理路径语义不可靠 → 改成按会话 id 定位：
 *   ① 快路径：`<projectsRoot>/<encodeProjectDir(cwd)>/<id>.jsonl` 存在 → 直接用它（零扫描）；
 *   ② 慢路径：扫 `projectsRoot` **一层**目录，谁下面有 `<id>.jsonl` 就是它（只扫一层、不递归）；
 *      目录**先按名字排序再截断**（`getChildren` 是文件系统枚举序，>500 时每次都可能不同）；
 *   ③ 都找不到 → `found=null`（调用方维持既有 fail-safe：回滚/分支拒绝并说明）。
 * 安全：id 过 sessionStore 白名单（`../` 类 id 进不来）；命中路径原样返回，不做 realpath
 * （realpath 会把后续的「替换原文件—还原」指到别名上，读写必须同一拼法）。
 *
 * 返回 `{found, capped}` 而不只是命中的那条：`/diag` 必须能区分「扫完了、确实没有」与
 * 「没扫完、被上限挡了」；回滚/分支那条路走下面的 `findClaudeSessionFile` 兼容壳（形状不变）。
 */
export async function lookupClaudeSessionFile(
  input: FindSessionFileInput,
  deps: { fs: RewindFs; log?(message: string): void },
): Promise<SessionFileLookup> {
  const id = String(input.claudeSessionId ?? "");
  if (!isSafeId(id)) {
    return { found: null, capped: false };
  }
  const fileName = `${id}.jsonl`;
  const quickDir = encodeProjectDir(String(input.cwd ?? ""));
  const quickPath = deps.fs.join(input.projectsRoot, quickDir, fileName);
  if (await deps.fs.exists(quickPath)) {
    return { found: { path: quickPath, projectDir: quickDir }, capped: false };
  }
  let names: string[];
  try {
    names = await deps.fs.listNames(input.projectsRoot);
  } catch (err) {
    deps.log?.(
      `lookupClaudeSessionFile: list ${input.projectsRoot} failed: ${String(err)}`,
    );
    return { found: null, capped: false };
  }
  // 顺序确定化：getChildren 给的是文件系统枚举序（>500 时每次拿到的 500 条都可能不同）。
  // 有 mtimeMs 能力 → **按 mtime 降序**（最近写过的项目目录更可能是目标，超上限时命中率显著更高），
  // 同 mtime 按目录名升序兜底；没有该能力 → 纯按目录名（确定性同样成立）。
  let sorted = [...names].sort(scanOrder);
  if (typeof deps.fs.mtimeMs === "function") {
    const stamp = new Map<string, number>();
    for (const name of sorted) {
      try {
        const ms = await deps.fs.mtimeMs(
          deps.fs.join(input.projectsRoot, name),
        );
        stamp.set(
          name,
          typeof ms === "number" && Number.isFinite(ms) ? ms : -1,
        );
      } catch {
        stamp.set(name, -1);
      }
    ...
    sorted = sorted.sort((a, b) => {
      const d = (stamp.get(b) ?? -1) - (stamp.get(a) ?? -1);
      return d !== 0 ? d : scanOrder(a, b);
    });
  }
  const capped = sorted.length > PROJECTS_SCAN_LIMIT;
  for (const name of sorted.slice(0, PROJECTS_SCAN_LIMIT)) {
    // 目录名里带分隔符/上跳的条目直接跳过（拼接后可能跑到 projectsRoot 外面）
    if (!name || name === quickDir || /[/\\]/.test(name) || name === "..") {
      continue;
    }
    const candidate = deps.fs.join(input.projectsRoot, name, fileName);
    if (await deps.fs.exists(candidate)) {
      return { found: { path: candidate, projectDir: name }, capped: false };
    }
  }
  if (capped) {
    deps.log?.(
      `lookupClaudeSessionFile: ${sorted.length} dirs under ${input.projectsRoot} > ${PROJECTS_SCAN_LIMIT}, scan capped`,
    );
  }
  return { found: null, capped };
}

/** 定位但只取命中结果（回滚/分支/快照路径用；扫满上限未命中 → null，与旧行为逐字一致） */
export async function findClaudeSessionFile(
  input: FindSessionFileInput,
  deps: { fs: RewindFs; log?(message: string): void },
): Promise<FoundSessionFile | null> {
  return (await lookupClaudeSessionFile(input, deps)).found;
}
...
export async function snapshotTurn(
  ...
  // 信任边界（安全复查 2026-09-11）：sessionId 来自桥消息，拼进路径前必须过白名单
  // （与 sessionStore 同一判据，防 ../ 类越界写）
  if (!isSafeId(input.sessionId)) {
    throw new Error(
...
export async function rewindToTurn(
  ...
  // 信任边界（同 snapshotDir）：id 先过白名单再进任何路径拼接
  if (!isSafeId(String(input.sessionId ?? ""))) {
    deps.log?.("rewind: unsafe session id → refused");
    return { ok: false, reason: "SNAPSHOT_MISSING", error: "会话 id 非法" };
  }
  if (rewindBusy) {
