 ...
 *   ③ 都找不到 → null（调用方维持既有 fail-safe：回滚/分支拒绝并说明）。
...
export async function findClaudeSessionFile(
  ...
  if (await deps.fs.exists(quickPath)) {
    return { path: quickPath, projectDir: quickDir };
  }
  let names: string[];
  try {
  ...
  for (const name of names.slice(0, PROJECTS_SCAN_LIMIT)) {
    ...
    if (await deps.fs.exists(candidate)) {
      return { path: candidate, projectDir: name };
    }
  }
  if (names.length > PROJECTS_SCAN_LIMIT) {
    deps.log?.(
      `findClaudeSessionFile: ${names.length} dirs under ${input.projectsRoot} > ${PROJECTS_SCAN_LIMIT}, scan capped`,
    );
  }
  return null;
}
...
export async function snapshotTurn(
  ...
  if (!isSafeId(input.sessionId)) {
    throw new Error(`snapshotTurn: unsafe session id: ${String(input.sessionId)}`);
  }
