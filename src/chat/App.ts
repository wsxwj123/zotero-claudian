...
import { scopeRefreshable, type ScopePickerState } from "./lib/scopePicker";
import { SCOPE_SELECTION_LABEL } from "../utils/scope";
...
/** R12-A：命令面板的来源标签（每项显示「名字 · 来源 · 描述」） */
const COMMAND_SOURCE_LABEL: Record<string, string> = {
  local: "本地",
  builtin: "内置",
  project: "项目",
  user: "用户",
...
export function App(props: AppProps): VNode<any> {
  ...
  const runLocalCommand = (action: string): void => {
    ...
    switch (action) {
      ...
      case "showHelp":
        ...
        return;
      case "showDiag": {
        // R9：/diag —— 宿主只读采集本机事实，报告文本随 diagReport 回执进 diag.text
        const r = openDiag(cur);
        props.store.set(r.state);
        props.bridge.send(r.msg);
        return;
      }
  ...
  // R12-A：点「+ 范围」只展开选项，不发请求（此刻用户多半什么都还没选）
  const onScopeOpen = (): void => {
    props.store.set(openScopePanel(props.store.get()));
  };
  ...
  // R12-C：0 篇提示里的「重试」→ 重发同一种请求
  const onScopeRetry = (): void => {
    const notice = props.store.get().scope.notice;
    if (notice) {
      onScopeRequest(notice.kind);
    }
  };

  // R12-D：chip 上的 ↻ → 重新解析当前选择（在途/分类 chip 时 refreshScope 回 null，不发）
  const onScopeRefresh = (): void => {
    const r = refreshScope(props.store.get());
    if (r) {
      props.store.set(r.state);
      props.bridge.send(r.msg);
    }
  ...
  // R8：「全页」= 原有独立工作台标签页（浮层撤掉后入口搬到面板顶栏）。
  // 只发一个无载荷动作，宿主自己决定怎么开（桥只认已注册实例，页面拿不到任何宿主能力）。
  const onOpenFullPage = (): void => {
    props.bridge.send({ type: "openFullPage" });
  };
  ...
  return h(
    ...
    h(SessionList, {
      ...
      // R10：会话区展开/收起——① 本地立刻改（视图态在 store 里，不等往返）② 宿主落 prefs
      //（只写不回执：重开面板时由 uiPrefs 还原，面板内不来回改，省得把用户刚点开的又收回去）
      onToggleExpand: (expanded: boolean) => {
        props.store.set({ ...props.store.get(), sessionsExpanded: expanded });
        props.bridge.send({ type: "setSessionsExpanded", expanded });
      },
    ...
    // R12-B：状态不再单独占一行 —— 交给 InputBox 与「＋范围 / 📎附件」同排
    h(InputBox, {
      statusNode: h(StatusLine, { state }),
    ...
    state.helpOpen
      ...
      : null,
    // R9：/diag 诊断报告弹层（内容来自宿主只读采集；可复制）
    state.diag
      ? h(DiagOverlay, {
          text: state.diag.text,
          onClose: () => props.store.set(closeDiag(props.store.get())),
        })
      : null,
  );
}
...
/**
 * R9：/diag —— 诊断报告弹层。内容全部来自宿主只读采集（不发给 CLI、不花 token）。
 * 用途是把「真机排障」从逐项人工核对降成「贴一段输出」；提示语里写明报告含本机路径与
 * 条目 key（用户自己贴的，公开场合请自行删减）。
 */
function DiagOverlay(props: {
  text: string | null;
  onClose: () => void;
}): VNode<any> {
  ...
  const ready = typeof props.text === "string" && props.text !== "";
  return h(
    "div",
    { class: "overlay", "data-testid": "diag-overlay" },
    h(
      "div",
      { class: "overlay-card" },
      h(
        "div",
        { class: "overlay-head" },
        h("span", { class: "overlay-title" }, "诊断报告"),
        h(
          "button",
          { class: "overlay-close", onClick: props.onClose, title: "关闭" },
          "×",
      ...
      h("div", { class: "overlay-hint" }, DIAG_HINT),
      h(
        "pre",
        { class: "overlay-text diag-text", "data-testid": "diag-text" },
        ready ? props.text : "采集中…",
      ),
      h(
        "div",
        { class: "overlay-foot" },
        ...
        h(
          "button",
          {
            ...
            disabled: !ready,
            onClick: () => {
              void copyToClipboard(props.text ?? "").then((ok) => {
                if (!ok) {
                  recordDiag("ui.diag copy failed（clipboard 两条通道都失败）");
                  return;
                }
                setCopied(true);
              });
...
/** 报告弹层提示语（红线要写在用户看得到的地方：贴之前自己过一眼） */
const DIAG_HINT =
  "报告含本机路径与文献条目 key，公开贴出前请自行删减；不含任何密钥或令牌（有值只写 set/unset）。";
...
function Header(props: {
  ...
  return h(
    ...
    h(
      ...
      h(
        "span",
        { class: "header-title", title: parts.join(" · ") || "Claude" },
      ...
      // R8：全页工作台（浮层撤掉后从这进；宿主开独立标签页）
      h(
        "button",
        {
          class: "header-btn",
          title: "在独立标签页中打开 Claude 工作台",
    ...
    // 用量行仍独立一行（11px 细行）：它文本长（本轮/缓存/累计/余额），并进标题行会把窄面板
    // 挤成 3 行 —— 实测 260px 下 3 行(80px) 反而不如独立行(≈42px)
    h(UsageRow, {
      state: props.state,
...
/**
 * 状态（PLAN §7.2 风险 5：CLI 冷启动最长 90s，等待期显示计时缓解焦虑）。
 * R12-B（用户反馈「把'范围''附件'和'就绪'放在同一行」）：这里只出**一段文本**，
 * 由 InputBox 的工具行把它和「＋范围 / 📎附件」摆在同一行；窄面板整行 nowrap + 省略号。
 */
function StatusLine(props: { state: ChatState }): VNode<any> {
  ...
  // 挤窄时优先牺牲这段文字（省略号），完整内容留在 tooltip 里
  return h(
    "span",
    { class: "status-text", "data-testid": "status-text", title: text },
    text,
  );
}
...
/** 粘贴 File → base64（**唯一**碰 FileReader 的地方；单测注入替身即可不碰 DOM） */
 ...
 * 由宿主按「缺少内容」拒绝并回人话原因。
 *
 * R8-④ 体积门：超 20MB 的文件**连 FileReader 都不进**（否则 100MB 的图会先被整段读进内存，
 * 拒绝时白烧几百 MB），原样交给 chipsAdd 出「超过单文件 20MB 上限」的提示。
 * `readBase64` 注入只是为了单测，运行时恒是 readFileBase64。
 */
export async function filesToByteAttachments(
  ...
  for (const file of list) {
    const base64 = withinAttachmentSizeGate(file.size)
...
function InputBox(props: {
  /** R12-B：状态文本节点（与「＋范围 / 📎附件」同排；由 App 组装，这里不关心内容） */
  statusNode: VNode<any>;
  disabled: boolean;
  ...
  // 用户反馈（截图）：「＋ 范围」与「📎 附件」竖着两行排 —— 两个按钮进**同一行**工具条；
  // 窄面板（CSS 容器查询，见 index.html 的 .tool-row）时只留图标，整行压到最小可用高度。
  // R12-B（用户反馈「把'范围''附件'和'就绪'放在同一行」）：状态文本也进这一行（行首），
  // 原来它自己占一行（.status 上下各一条 padding + 边框），合并后输入区少一行。
  const scopeBtn = h(
    "button",
    {
      class: "scope-btn-add btn-icon-text",
      title: `按范围注入：当前分类全部 / ${SCOPE_SELECTION_LABEL}（先给清单，Claude 需要时自己读）`,
      "aria-label": "选择注入范围",
      onClick: props.onScopeOpen,
    },
    icon("plus", 14),
    h("span", { class: "btn-label" }, "范围"),
  );
  const attachBtn = h(
    "button",
    {
      class: "attach-btn btn-icon-text",
      "data-testid": "attachment-pick",
      title: "选择文件作为附件（宿主弹原生选择器；也可以直接粘贴截图/文件）",
      "aria-label": "添加附件",
      onClick: () => props.onPickAttachments(),
    },
    icon("paperclip", 14),
    h("span", { class: "btn-label" }, "附件"),
  );
  return h(
    "div",
    { class: "input-wrap" },
    // R7-D：范围 chip（`分类名 · N 篇`；截断时标注）+ 「+ 范围」/「📎 附件」工具行
    // R12-B：行首是状态（就绪/等待 CLI…）——「就绪 · ＋范围 · 📎附件」同一行
    h(
      "div",
      { class: "chips tool-row", "data-testid": "scope-chips" },
      props.statusNode,
      scopeChip
        ? h(
            "span",
            ...
            h(
              "span",
              { class: "chip-text" },
              props.scope.refreshing
                ? `${scopeChip.label} · 解析中…`
                : `${scopeChip.label} · ${scopeChip.count} 篇${scopeChip.truncated ? "（已截断至 40 篇）" : ""}`,
            ),
            // R12-D：选择是快照，用户可能刚改了选择 → 给一枚 ↻（分类 chip 不需要）
            scopeRefreshable(props.scope)
              ? h(
                  "button",
                  {
                    class: "chip-x",
                    "data-testid": "scope-refresh",
                    title: "重新解析当前选择（书库中选中的文献）",
                    "aria-label": "重新解析范围",
                    onClick: props.onScopeRefresh,
                  },
                  "↻",
                )
              : null,
    ...
    // 范围选项面板：只在展开时占一行（并排塞进工具行会把整行撑成两行——工具行恒 nowrap）
    props.scope.open
      ? h(
          "div",
          { class: "chips", "data-testid": "scope-picker-row" },
          h(
            "div",
            { class: "scope-picker", "data-testid": "scope-picker" },
            // R12-B：两条路径各一行 —— 主按钮 + 灰色小字说清「怎么选」
            h(
              "div",
              { class: "scope-opt-row" },
              h(
                ...
                {
                  class: "scope-opt",
                  "data-testid": "scope-opt-collection",
            ...
            h(
              ...
              h(
                ...
                {
                  class: "scope-opt",
                  "data-testid": "scope-opt-selection",
              ...
              h(
                "span",
                { class: "scope-hint" },
                "先去左侧文献列表里选中若干条（⌘/Ctrl 多选），再点这里",
              ),
            ),
            // R12-C：0 篇就地提示 + 重试（不落一枚 0 篇的废 chip）
            props.scope.notice
              ? h(
                  "div",
                  { class: "scope-opt-row", "data-testid": "scope-notice" },
                  h("span", { class: "chip-notice" }, props.scope.notice.text),
                  h(
                    "button",
                    {
                      class: "scope-opt",
                      "data-testid": "scope-retry",
                      onClick: props.onScopeRetry,
                    },
                    "重试",
                  ),
                  h(
      ...
      : null,
    // R7-J：附件 chips（图标 + 文件名 + 大小 + 删除）；**没有附件就整行不渲染**
    // （空行白占 ~32px 垂直空间，用户反馈「上下占用太大」）
    props.attachments.items.length > 0 || props.attachments.notice
      ? h(
          "div",
                ...
                h(
                  ...
                  h(
                    ...
                    h(
                      "span",
                      { class: "command-tag" },
                      COMMAND_SOURCE_LABEL[cmd.source] ?? "用户",
                    ),
                  ),
    ...
    h(
      ...
      h("textarea", {
        ...
        onInput: (e: Event) => {
          ...
          syncMention(el.value, caret);
          // R9 修：`/` 面板的语境同步此前**从未接线**（syncCommands 定义了却没有调用点）
          // → 打字打 `/` 永远不弹命令面板（/help、/diag 都够不着）。与 @ 提及同一处、同一时机。
          syncCommands(el.value, caret);
        },
