...
/**
 * 体积门（纯逻辑，R8-④）：单文件超 20MB 一律**不读字节**——粘贴一张 100MB 的图不该先整段
 * 读成 base64（页面内存炸 + 秒级卡顿）再被拒。读那侧（App.ts 粘贴路径）与收那侧
 * （attachmentChipsAdd）共用这一条判据，宿主侧还有第三道（attachments.ts），三处同口径。
 * 非数字/非有限 → 放行（读不出来自然无字节，交给宿主按「缺少内容」拒绝）。
 */
export function withinAttachmentSizeGate(sizeBytes: unknown): boolean {
  return (
...
export function attachmentChipsAdd(
  ...
  for (const file of Array.isArray(files) ? files : []) {
    ...
    if (!withinAttachmentSizeGate(file.sizeBytes)) {
