...
/**
 * 并入（历史只增不减）：页面侧（mergeHostEntries）与宿主侧（inputHistoryStore.save）共用的
 * 唯一实现——两处各写一份曾在「头部被 50 条上限挤掉 / 新来的是残份」时退化成整段拼接
 *（复查修-7：留重复段、且把旧条目排到最新之后，↑ 翻出来的“最新一条”是旧的）。
 *
 * 两份都是同一条时间线的窗口（各自可能被上限剪掉过头部，或来自刚重载还没收到宿主的残份）：
 * 按**最长公共连续段**对齐，以它在 incoming 里的终点为界，只补 incoming 中 stored 尾部
 * 之后的部分。找不到公共段（两份互不相识）→ 照旧拼接，由上方归一 + 上限裁剪兜底。
 * 幂等：同一 incoming 并两次结果不变（第二次的公共段落在尾部，补不出新东西）。
 */
export function mergeHistoryEntries(
  stored: string[],
  incoming: unknown,
): string[] {
  const base = normalizeHistoryEntries(stored);
  const add = normalizeHistoryEntries(incoming);
  if (base.length === 0 || add.length === 0) {
    return base.length === 0 ? add : base;
  }
  // 最长公共连续段：两个数组都 ≤ 50，双循环代价可忽略
  let len = 0;
  let addEnd = 0;
  ...
  return normalizeHistoryEntries(
    len === 0 ? [...base, ...add] : [...base, ...add.slice(addEnd)],
  );
}
 ...
 * - 否则以宿主那份为底、按 mergeHistoryEntries 对齐并入本页面的新尾部；
 * - 归一（相邻去重 + 上限 50）后内容不变 → 返回原对象（渲染期反复调用不自激）。
 */
export function mergeHostEntries(
  ...
  const merged = mergeHistoryEntries(host, local);
  if (
