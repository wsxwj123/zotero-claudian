...
//
// R8 折叠口径收窄（用户反馈「为什么现在 AI 的消息会自动折叠？应该是**代码块包裹的内容**自动折叠，
// **用户的消息超过一定长度后**自动折叠」）——按角色 + 按块判：
//   · AI 回复正文 → **恒不折**（messageBodyCollapsed 对 assistant 恒 false）；
//   · AI 回复里的代码块 → 超 CODE_COLLAPSE_LINE_THRESHOLD 行按**块**折（渲染侧见 markdown.foldCodeBlocks，
//     折叠是原生 details 的 DOM 视图态：重画/切会话即复位，同一条消息里各块各自独立）；
//   · 用户消息 → 仍按 COLLAPSE_LINE_THRESHOLD 行整条折（沿用 R7-F 语义）。
//
...
/** 折叠阈值（行数；**超过**才默认折叠）——只作用于**用户消息**（整条折） */
export const COLLAPSE_LINE_THRESHOLD = 12;
/** 代码块折叠阈值（行数；**超过**才默认折叠）——只作用于 **AI 回复里的代码块**（按块折，R8） */
...
/**
 * 代码行数（末尾换行不算新行 —— markdown 渲染器会给每个代码块补一个尾部 \n，
 * 直接 split 会把「19 行的块」数成 20 行）。
 */
export function codeLineCount(code: unknown): number {
...
/** 代码块超长判定：行数 > 20（硬条件，同 isOverflowing 口径） */
export function isCodeOverflowing(code: unknown): boolean {
  return codeLineCount(code) > CODE_COLLAPSE_LINE_THRESHOLD;
}

/**
 * 正文是否折叠（R8 按角色收窄）：user → 超 12 行整条折；assistant → **恒不折**
 * （正文永不折；长代码块由渲染侧按块折）。divider/异常输入 → 不折。
 * 流式期间由调用方跳过（回答在增长，折起来每来一个 delta 就跳一次）。
 */
export function messageBodyCollapsed(
  state: MessageActionState,
  index: number,
  turn: unknown,
): boolean {
  ...
  return messageCollapsed(state, index, t.text);
}
