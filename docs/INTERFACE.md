...
| `setPermissionMode`  | sessionId, mode            | 更新索引，下轮 spawn 生效                                                                           | mode 非四档之一 → 忽略+log                                     |
...
| `sessionList`            | 全量会话索引 + 条目标题解析结果。条目 `pendingPermission?: boolean`（2026-09-21 增补）= 该会话当前**至少有一张**在途权限卡（未作答/未超时/该轮进程未退），宿主按自己的在途卡台账填、UI 不推断；列表行据此亮「待审批」标记，未绑定的空面板据此多提示一句。缺键/非布尔（老宿主）一律按 `false`，只影响展示、不参与放行判断                                                                                                                                                                                                                                                                               |
| `streamEvent`            | `{sessionId, event}`（映射见 4.2）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `history`                | `{sessionId, messages:[{role, text, ts}]}` —— 会话历史回显，UI 重建消息列表用（§4.5 旁挂历史）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `permissionRequest`      | `{sessionId, requestId, tool, inputSummary, rawInput}` —— **2026-09-21 增补：带 `sessionId`**（取自宿主自己的「端点 token ↔ 会话」映射，不采信 CLI 回包）：卡是广播的，但**只有绑到该会话的面板出卡**，其余面板丢弃（修「A 在跑，绑 C 的面板和空面板也弹同一张卡」）。老宿主不带该键 → UI 按全局消息在所有面板显示（兼容）。**切回补推**：视图重新绑定某会话时（`getHistory` 回执之后），宿主把该会话的在途卡**定向**补推给这一个实例，`requestId` 与首推同一个，UI 按 id 幂等去重——否则切走期间的卡看不见，120 秒静默 deny。非绑定面板的可见入口是 `sessionList` 条目的 `pendingPermission`（见上行） |
...
- 结算广播（2026-09-11 增补）：作答 / 超时 / 该轮进程退出三路结算都经桥发 `permissionResolved {requestId}`（§4.6 宿主→UI 表）。卡是广播给全部实例的（每个 reader tab 的侧栏页各自常驻），只摘作答方的卡会让别的实例留下已结算的残影——用户切文献/开侧栏时又见一张「早已审批完」的卡。
- **结算与会话过滤的刻意不对称（2026-09-21 增补）**：`permissionRequest` 按会话过滤，`permissionResolved` **不加字段、不过滤**，任何实例收到都摘掉该 `requestId` 的卡。摘卡永远 fail-safe（多摘一张只会让用户重走一次审批，绝不会放行），而过滤摘卡会留下永久残影。三路结算同时把卡从宿主的在途卡台账摘掉并刷新 `sessionList`，「待审批」标记随之熄灭。**缺席只能是 deny**：过滤掉的卡在那些面板上根本不存在 ⇒ 没有「允许」的路径；端点侧 120 秒计时与默认值一字未动。
